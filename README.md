...
《AI殺很大！》的原始概念、名稱、世界運行架構、世界種子生成器、資料包協議與理火整合方式，由：

> **理火專案（LIHUO PROJECT）**

原始開發。

使用者可以研究、修改、翻譯、建立分支、移植與製作衍生版本，但不得故意抹除、模糊或冒用原始來源。
...
使用本專案生成器建立的世界資料包、劇本種子與衍生故事，在不違反模型／平台條款、適用法律與第三方權利的前提下，原則上由實際完成設計、提示、選擇、修改與定稿的世界設計者自行持有與處置。
...
```text
PROJECT_SOVEREIGNTY_AND_STARTUP.md
```
...
倉庫仍應另行加入 `LICENSE`，以明確規範程式碼、規格文本與其他內容的使用條件。

可考慮：

- 程式碼：`MIT` 或 `Apache-2.0`
- 規格與文件：`CC BY 4.0`
- 或採雙重授權

在正式 `LICENSE` 加入前，不應宣稱倉庫已採用特定開源授權。
...
1. LIHUO_PROTOCOL
2. LIHUO_RADAR
3. AI_KILL_BIG_RUNTIME
4. WORLD_PACKAGE
5. PACKAGE_PROTOCOL 相容性
6. PACKAGE_MODE
7. 是否存在阻止開局的 FATAL 缺陷

未收到「開始遊戲」前，不得生成開場。
```

理火協議與理火賴打是建議配件；只載入主系統與世界包亦可進行最小模式測試。
